...
export default function PagamentoFinalElite() {
  ...
  const finalizarPedido = async () => {
    ...
    try {
      ...
      // 🛠️ AJUSTE DE VÍNCULO PARA O FINANCEIRO (Mogu Mogu)
      // Isso garante que a API de fechamento encontre o restaurante correto
      const itensVinculados = cart.map(item => ({
        ...item,
        restaurantId: item.restaurantId || "rest_1" // Vincula ao Burger Master no Firebase
      }));

      const dadosPedido = {
        clienteId: user?.uid || "anonimo",
        clienteNome: user?.displayName || "Cliente Guapiara",
        itens: itensVinculados, // Usa a lista com os IDs vinculados
        valores: {
        ...
        repasseConfirmado: false, // 💰 Para o seu Dashboard Financeiro
        criadoEm: hoje // Data formatada para facilitar a busca do índice
      };

      // 1. Grava o pedido no Firestore
      await addDoc(collection(db, "orders"), dadosPedido);

      // 2. ATUALIZAÇÃO DA LINHA DO DIA
      if (idLinha) {
    ...
    <main className="min-h-screen bg-zinc-950 text-white p-6 font-sans max-w-md mx-auto flex flex-col justify-center">
      ...
      <div className="bg-white text-black p-8 rounded-[40px] mb-8 shadow-2xl">
        <div className="flex justify-between text-[10px] font-black uppercase opacity-30 mb-2">
          ...
          <span>R$ {cart.reduce((acc, i) => acc + (Number(i.price) || 0), 0).toFixed(2)}</span>
        </div>
        <div className="flex justify-between text-[10px] font-black uppercase opacity-30 mb-2">
          <span>Logística ({logistica.pesoTotal} pts)</span>
      ...
      <div className="grid grid-cols-2 gap-3 mb-10">
        {["Pix", "Cartão"].map(m => (
          <button 
            ...
            className={`py-5 rounded-3xl font-black uppercase italic text-[10px] border-2 transition-all ${formaPagamento === m ? "border-red-600 bg-red-600 text-white shadow-lg shadow-red-600/20" : "border-zinc-800 text-zinc-500"}`}
          >
            {m}
          </button>
        ))}
