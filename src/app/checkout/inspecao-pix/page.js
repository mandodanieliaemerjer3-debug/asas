...
export default function InspecaoPixPro() {
  ...
  useEffect(() => {
    async function analisar() {
      ...
      try {
        ...
        // 1. Procura por números no formato 0,00 ou 0.00
        // 2. Limita a no máximo 5 dígitos antes da vírgula (evita CPFs e IDs longos)
        // 3. Verifica se tem um "r$" ou espaço por perto
        const regexValores = /(?:r\$\s*|(?:\s|^))(\d{1,5}(?:[.,]\d{2}))/g;
        let valoresEncontrados = [];
        let m;

        while ((m = regexValores.exec(txt)) !== null) {
          const valorLimpo = m[1].replace(",", ".");
          const num = parseFloat(valorLimpo);
          if (!isNaN(num)) valoresEncontrados.push(num);
        }

        // 🎯 SELEÇÃO INTELIGENTE:
        // Se a IA ler vários números, ela escolhe o que for MAIS PRÓXIMO do valor do pedido
        ...
        setAnalise({
          temPalavras: txt.includes("comprovante") || txt.includes("pix") || txt.includes("pago") || txt.includes("mercado"),
  ...
  if (erro) return (
    <main className="min-h-screen bg-black text-white flex flex-col items-center justify-center p-10 text-center">
      ...
      <button onClick={() => router.back()} className="bg-red-600 px-10 py-4 rounded-2xl font-black uppercase">Voltar</button>
    </main>
  );
  ...
  return (
    <main className="min-h-screen bg-black text-white p-8 flex items-center justify-center">
      <div className="bg-zinc-900 border border-zinc-800 p-8 rounded-[40px] w-full max-w-sm space-y-8 shadow-2xl">
        ...
        >
          {analise.valorConfere ? "LIBERAR PEDIDO ➔" : `FALTAM R$ ${Math.abs(analise.diferenca).toFixed(2)}`}
        </button>
      </div>
    </main>
  );
