...
export default function CheckoutPage() {
  ...
  const [bebidasCart, setBebidasCart] =
    useState([]);

  const [docesCart, setDocesCart] =
    useState([]);
  ...
  // ITENS AGRUPADOS
  // =========================
  const pratosAgrupados =
    agruparItens(cart, "name");

  const bebidasAgrupados =
    agruparItens(
      bebidasCart,
      "nome"
    );

  const docesAgrupados =
    agruparItens(
      docesCart,
      "nome"
    );
  ...
  const removerItem = ({
    ...
    const index =
      lista.findLastIndex(comparador);

    if (index === -1) {
      return;
    }

    const novo = lista.filter(
      (_, i) => i !== index
    );
