...
import parceirosData from "../data/parceiros.json"; // CONEXÃO COM OS PARCEIROS
...
import ParceirosLocais from "../components/ParceirosLocais"; // COMPONENTE DE CLIQUES
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(doc(db, "users", user.uid), (snap) => {
      if (snap.exists()) {
        setUserProfile({ uid: user.uid, ...snap.data() }); // Garante o UID para o sistema de cliques
      }
    });
  ...
  return (
    <main className="min-h-screen bg-white pb-44 font-sans max-w-md mx-auto shadow-2xl relative border-x border-gray-100 overflow-x-hidden">
      ...
      {/* CARROSSEL DE NOTAS (SLIM CENTRALIZADO) */}
      <CarrosselNotas notas={notas} />

      {/* NOVO SISTEMA DE PARCEIROS E CLIQUES (SEGMENTADO) */}
      <ParceirosLocais 
