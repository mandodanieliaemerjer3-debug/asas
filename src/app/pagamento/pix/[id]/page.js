...
export default function PixCompletoPage() {
  ...
  const finalizarPagamento = async () => {
    ...
    try {
      ...
      if (arquivo.type.startsWith("image/")) arquivoFinal = await comprimirImagem(arquivo);
  ...
  return (
    <main className="min-h-screen bg-gray-50 max-w-md mx-auto font-sans pb-20">
      <header className="bg-white p-6 rounded-b-[40px] shadow-sm border-b text-center">
        ...
        <h1 className="font-black italic uppercase text-2xl tracking-tighter leading-none">Pagamento Pix</h1>
        <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mt-1">Pedido: {id?.slice(-6)}</p>
      </header>
      ...
      <div className="p-4 space-y-6">
        <section className="bg-white p-6 rounded-[32px] shadow-sm border border-gray-100 text-center relative overflow-hidden">
          ...
          <p className="text-[10px] font-black text-gray-400 uppercase mb-4">Valor exato da transferência</p>
          <p className="text-4xl font-black text-gray-900 italic mb-6 leading-none tracking-tighter">R$ {pedido?.valores?.total.toFixed(2)}</p>
          ...
          <div onClick={copiarChave} className="bg-gray-50 p-4 rounded-2xl border-2 border-dashed border-gray-200 cursor-pointer active:scale-95 transition">
            <p className="text-[9px] font-black text-gray-400 uppercase mb-1 tracking-widest">Chave Pix do Restaurante</p>
            <p className="font-mono text-xs text-gray-800 break-all">{chavePix || "Indisponível"}</p>
            {copiado && <p className="text-teal-500 text-[8px] font-black uppercase mt-2 animate-bounce">Copiado para o seu teclado!</p>}
          </div>
        </section>

        <section className="bg-white p-6 rounded-[32px] shadow-sm border border-gray-100">
          <h2 className="text-[10px] font-black uppercase text-gray-400 mb-4 text-center tracking-widest leading-none">Anexar Comprovante</h2>
          <label className="relative flex flex-col items-center justify-center w-full min-h-[160px] bg-gray-50 rounded-[28px] border-2 border-dashed border-gray-200 cursor-pointer hover:bg-gray-100 transition overflow-hidden">
            ...
            <div className="relative z-10 flex flex-col items-center text-center px-4">
              ...
              <p className="text-[10px] font-black text-gray-600 uppercase leading-tight tracking-tighter">
                {arquivo ? arquivo.name : "Clique para anexar o comprovante"}
              </p>
            </div>
            <input type="file" className="hidden" accept="image/*,application/pdf" onChange={handleFileSelect} />
          </label>
          <button onClick={finalizarPagamento} disabled={uploading || !arquivo} className="w-full mt-6 bg-red-600 text-white py-5 rounded-[24px] font-black uppercase italic shadow-xl shadow-red-100 active:scale-95 transition disabled:bg-gray-200 tracking-tighter">
