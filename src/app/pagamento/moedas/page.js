...
export default function PagamentoMoedasPage() {
  ...
  const moedasProdutos =
    pedido?.itens?.reduce(
      (acc, item) => acc + (item.coinPrice || 0),
      0
    ) || 0;

  const freteEmMoedas =
    (pedido?.valores?.taxaEntrega || 0) * 200;
  ...
  if (loading)
    return (
      <div className="p-20 text-center font-black animate-bounce italic uppercase tracking-widest">
        Calculando Saldo...
      </div>
    );
  ...
  return (
    <main className="min-h-screen bg-gray-50 max-w-md mx-auto p-6 font-sans flex flex-col items-center justify-center">
      <div className="w-20 h-20 bg-yellow-100 text-yellow-600 rounded-[32px] flex items-center justify-center text-4xl mb-4 border-4 border-white shadow-sm">
        🪙
      </div>

      <h1 className="font-black italic uppercase text-2xl tracking-tighter text-gray-900 leading-none">
        Confirmar Troca
      </h1>

      <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mt-2 mb-8">
        Pagamento com Moedas Mogu
      </p>
      ...
      <section className="w-full bg-white p-8 rounded-[40px] shadow-xl border border-gray-100 text-center">
        ...
        <p className="text-5xl font-black text-orange-600 mb-8 italic tracking-tighter leading-none">
