...
export default function PagamentoDinheiroPage() {
  ...
  const finalizarNoDinheiro = async () => {
    ...
    try {
      ...
      const trocoCalculado = precisaTroco
        ? (parseFloat(valorTrocoPara) - totalPedido)
        : 0;
      ...
      await updateDoc(doc(db, "orders", id), {
        ...
        detalhesTroco: {
          ...
          levarPara: precisaTroco
            ? parseFloat(valorTrocoPara)
            : totalPedido,
  ...
  return (
    <main className="min-h-screen bg-white p-6 flex flex-col items-center font-sans max-w-md mx-auto">
      <div className="w-16 h-16 bg-emerald-50 text-emerald-600 rounded-3xl flex items-center justify-center text-3xl mb-4">
        💵
      </div>
      
      <h1 className="font-black italic uppercase text-xl tracking-tighter text-zinc-900 leading-none">
        Pagamento em Dinheiro
      </h1>

      <p className="text-[10px] font-black text-zinc-400 uppercase mt-2 mb-8 tracking-widest">
        Total: R$ {pedido?.valores?.total.toFixed(2)}
      </p>

      <div className="w-full space-y-6 mb-8">
        <p className="text-[11px] font-black uppercase italic text-zinc-800 text-center">
        ...
        <div className="grid grid-cols-2 gap-3">
          <button 
            ...
            className={`py-5 rounded-3xl font-black uppercase italic text-[10px] border-2 transition-all ${
              precisaTroco === true
        ...
        {precisaTroco && (
          <div className="animate-in fade-in slide-in-from-top-2 duration-300">
            <p className="text-[9px] font-bold text-zinc-400 uppercase mb-2 ml-4">
              Troco para quanto?
            </p>
            <input 
              type="number" 
      ...
      <div
        className={`p-6 rounded-[35px] border-2 mb-10 transition-colors ${
          aceitaPixSemTroco
      ...
      >
        <label className="flex items-start gap-4 cursor-pointer">
          <input
            type="checkbox"
            checked={aceitaPixSemTroco}
            onChange={(e) => setAceitaPixSemTroco(e.target.checked)}
            className="w-5 h-5 mt-1 accent-emerald-600"
          />
          <p className="text-[9px] font-black uppercase leading-tight italic">
            Se o motoboy não tiver troco na hora, aceito pagar via Pix ✅
          </p>
        </label>
      </div>
      ...
      <button 
        ...
        disabled={
          precisaTroco === null ||
          (precisaTroco && !valorTrocoPara) ||
          !aceitaPixSemTroco ||
          processando
        }
        className={`w-full py-7 rounded-[35px] font-black uppercase italic text-xs shadow-2xl transition-all active:scale-95 ${
          (!aceitaPixSemTroco || processando)
