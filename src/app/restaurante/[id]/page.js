...
export default function PaginaExclusivaRestaurante() {
  ...
  // =========================
  // 🔥 AUTO-MODAL INTELIGENTE (Correção)
  // =========================
  useEffect(() => {
    const pendente = sessionStorage.getItem("produto_pendente_configuracao");
    
    // Só tenta abrir se houver algo pendente E o cardápio do Firebase já estiver carregado
    if (pendente && cardapio.length > 0) {
      const itemPendente = JSON.parse(pendente);
      
      // Procura o produto RICO no cardápio usando o ID ou o Nome (como garantia)
      const produtoCompleto = cardapio.find(p => 
        p.id === itemPendente.id || p.name === itemPendente.name
      );

      if (produtoCompleto) {
        setProdutoSelecionado(produtoCompleto);
        setModalAberto(true);
        // Limpa a memória para não abrir sozinho novamente
        sessionStorage.removeItem("produto_pendente_configuracao");
      }
    }
  }, [cardapio]);
